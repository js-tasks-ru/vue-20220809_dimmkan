  ...
  <form class="meetup-form" @submit.prevent="handleSubmit">
    ...
    <div class="meetup-form__aside">
      <div class="meetup-form__aside-stick">
        ...
        <ui-button variant="secondary" block class="meetup-form__aside-button" data-test="cancel" @click="handleCancel"
          >Отмена</ui-button
        >
...
import { uploadImage } from '../../../06-wrappers/05-UiImageUploader/ImageService';
...
export default {
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        if (this.localMeetup.imageToUpload) {
          const result = await uploadImage(this.localMeetup.imageToUpload); // Симулируем загрузку на сервер
          this.localMeetup.image = result.image;
        }
